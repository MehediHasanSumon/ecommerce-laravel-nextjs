...
import {
  ...
  selectPaymentBannerEnabled,
  selectPaymentBannerImage,
  selectPaymentBannerTitle,
...
function renderBrandIcon(icon: string) {
  ...
  switch (normalized) {
    ...
    default:
      return (
        <span className="text-xs font-bold uppercase">{normalized.slice(0, 1)}</span>
      );
  }
}
...
function DefaultPaymentMethods() {
  return (
    <div className="flex flex-wrap items-center gap-1.5 rounded-xl border border-border/80 bg-muted/30 p-2.5">
      <span className="flex h-7 items-center justify-center rounded bg-background px-2 font-black italic tracking-tighter text-[#1A1F71] text-xs border border-border/60">
        VISA
      </span>
      <span className="flex h-7 items-center justify-center gap-0.5 rounded bg-background px-2 border border-border/60">
        <span className="h-3.5 w-3.5 rounded-full bg-[#EB001B] opacity-90" />
        <span className="-ml-2 h-3.5 w-3.5 rounded-full bg-[#F79E1B] opacity-90" />
      </span>
      <span className="flex h-7 items-center justify-center rounded bg-[#E2136E] px-2 font-bold text-[11px] text-white">
        bKash
      </span>
      <span className="flex h-7 items-center justify-center rounded bg-[#F7941D] px-2 font-bold text-[11px] text-white">
        Nagad
      </span>
      <span className="flex h-7 items-center justify-center rounded bg-background px-2 text-[11px] font-semibold text-foreground border border-border/60">
        Cash on Delivery
      </span>
    </div>
  );
...
export function Footer() {
  ...
  const paymentBannerImage = useSettingsStore(selectPaymentBannerImage);
  const paymentBannerEnabled = useSettingsStore(selectPaymentBannerEnabled);
  const paymentBannerTitle = useSettingsStore(selectPaymentBannerTitle);
  ...
  const excludedFooterLinks = new Set([
    "/order-tracking",
    "/brands",
    "/deals",
    "/offers",
  ...
  const footerLinks = configuredFooterLinks.filter(
    (link) =>
      !excludedFooterLinks.has(link.href.toLowerCase()) &&
      !excludedFooterLinks.has(link.label.toLowerCase()),
  );
  ...
  async function handleSubscribe(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setMessageType("error");
      setMessage(
        maybeAxios.response?.data?.errors?.email?.[0] ??
        ...
        <div className="grid grid-cols-2 gap-x-6 gap-y-8 md:grid-cols-2 lg:grid-cols-6 lg:gap-8">
          ...
          <div className="col-span-2 lg:col-span-2">
            ...
            <form
              ...
              className="flex max-w-sm flex-col gap-2 sm:flex-row lg:flex-col xl:flex-row"
            >
              <input
                type="email"
              ...
              <button
                disabled={submitting}
                className="whitespace-nowrap rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-primary-foreground transition-colors hover:bg-primary/90 disabled:cursor-not-allowed disabled:opacity-70"
              >
            ...
            {message ? (
              <p
                className={`mt-3 text-xs ${messageType === "success" ? "text-muted-foreground" : "text-destructive"}`}
              >
                {message}
              </p>
            ) : (
            ...
            {/* Payment Banner (Under Stay in the Loop) */}
            {paymentBannerEnabled ? (
              <div className="mt-6 max-w-sm">
                <h3 className="mb-3 text-sm font-semibold">
                  {paymentBannerTitle || "We accept"}
                </h3>
                {paymentBannerImage ? (
                  <div className="overflow-hidden rounded-xl border border-border bg-card/60 p-2.5">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img
                      src={paymentBannerImage}
                      alt={paymentBannerTitle || "Payment methods banner"}
