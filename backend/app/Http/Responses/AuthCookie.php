...
class AuthCookie
{
    ...
    {
        return self::make(config('auth_api.refresh_cookie_name'), $token, config('auth_api.refresh_token_expiration_minutes'));
    }
    ...
    {
        return cookie()->forget(config('auth_api.refresh_cookie_name'));
    }
