...
export const productModuleConfigs: Record<ProductModule, ModuleConfig> = {
  ...
  "shipping-methods": {
    ...
    description: "Manage shipping methods, charges, delivery types, and storefront availability.",
    createLabel: "Create Shipping Method",
    defaultSort: "sort_order",
    statuses: commonStatus,
    fields: [
      { name: "name", label: "Shipping Name", type: "text" },
      { name: "slug", label: "Shipping Slug", type: "text" },
      { name: "description", label: "Description", type: "textarea", optional: true },
      { name: "charge", label: "Shipping Charge", type: "number" },
      { name: "delivery_type", label: "Delivery Type", type: "text" },
      { name: "estimated_delivery_time", label: "Estimated Delivery Time", type: "text", optional: true },
      { name: "sort_order", label: "Sort Order", type: "number", optional: true },
      { name: "status", label: "Status", type: "select", options: commonStatus },
    ],
    columns: [
      { key: "name", label: "Name", sortable: true, render: (item) => <span className="font-semibold">{String(item.name ?? "")}</span> },
      { key: "slug", label: "Slug", sortable: true },
      { key: "charge", label: "Charge", sortable: true, render: (item) => money((Number(item.charge ?? 0) * 100).toString(), undefined) },
      { key: "delivery_type", label: "Delivery Type", sortable: true },
      { key: "estimated_delivery_time", label: "ETA" },
