...
type DraftItem = { key: string; product: CreateOrderProduct | null; product_variant_id: string; quantity: string; unit_price: string; discount: string };
...
const emptyItem = (): DraftItem => ({ key: crypto.randomUUID(), product: null, product_variant_id: "", quantity: "1", unit_price: "", discount: "" });
...
function AdminOrderForm({ orderNumber }: { orderNumber?: string }) {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      setPaymentMethod(optionResponse.data.payment_methods[0]?.gateway ?? "cash_on_delivery");
      setOrderStatus(optionResponse.data.statuses.order[0] ?? "pending");
      if (!orderResponse) return;

      const order = orderResponse.data.order;
      const productResponse = await orderManagementService.searchProducts("", order.items.map((item) => Number(item.productId)).filter(Boolean));
      const products = productResponse.data.products;
      if (!active) return;
      setCustomerType(order.userId ? "registered" : order.guestCustomerId ? "guest" : "new_guest");
      setCustomerId(String(order.userId ?? order.guestCustomerId ?? ""));
      setCustomer({ name: order.customer.name ?? "", email: order.customer.email ?? "", phone: order.customer.phone ?? "" });
      ...
      setCouponDiscount(String(Math.max(0, order.summary.couponDiscount - Number(order.additionalDiscount || 0)) || ""));
    ...
    }).catch((error) => toast.error(toAppError(error).message)).finally(() => active && setLoading(false));
    return () => { active = false; };
  }, [orderNumber]);

  useEffect(() => { if (sameAddress) setShipping({ ...billing }); }, [billing, sameAddress]);
  ...
  async function submit(event: React.FormEvent) {
    ...
    if (items.some((item) => !item.product)) { toast.error("Select a product for every row."); return; }
    setSaving(true);
    const payload = {
      ...
      items: items.map((item) => ({ product_id: item.product?.id, product_variant_id: item.product_variant_id ? Number(item.product_variant_id) : null, quantity: Number(item.quantity), unit_price: Number(item.unit_price || 0), discount: Number(item.discount || 0) })),
    ...
    try {
      const response = editing && orderNumber
        ? await orderManagementService.fullUpdate(orderNumber, payload)
        : await orderManagementService.create(payload);
      toast.success(editing ? "Order updated successfully." : "Order created successfully.");
  ...
  if (loading || !options) return <div className="h-64 animate-pulse rounded-lg bg-muted" />;

  return <form onSubmit={submit} className="space-y-5">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><Link href={editing && orderNumber ? `/admin/orders/${encodeURIComponent(orderNumber)}` : "/admin/orders"} className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground"><ArrowLeft className="h-4 w-4" /> {editing ? "Back to Order" : "Back to Orders"}</Link><h1 className="mt-3 text-2xl font-extrabold">{editing ? "Edit Order" : "Create Order"}</h1><p className="mt-1 text-sm text-muted-foreground">{editing ? orderNumber : "Create a phone or admin order using the shared checkout workflow."}</p></div><Button type="submit" isLoading={saving} icon={<Save className="h-4 w-4" />}>{editing ? "Save Changes" : "Create Order"}</Button></div>
    ...
    <Panel title="Products"><div className="space-y-3">{items.map((item) => <OrderItemRow key={item.key} item={item} onChange={updateItem} onRemove={(key) => setItems((current) => current.filter((entry) => entry.key !== key))} />)}</div><Button type="button" size="sm" variant="secondary" className="mt-4" icon={<Plus className="h-4 w-4" />} onClick={() => setItems((current) => [...current, emptyItem()])}>Add Product</Button></Panel>
    <div className="grid gap-4 lg:grid-cols-[1fr_360px]"><Panel title="Order Information"><div className="grid gap-3 md:grid-cols-2"><FieldSelect label="Shipping Method" value={shippingMethodId} values={options.shipping_methods.map((item) => [String(item.id), `${item.name} · ${formatPrice(item.rate)}`])} onChange={(value) => { setShippingMethodId(value); setShippingCharge(String(options.shipping_methods.find((item) => String(item.id) === value)?.rate ?? 0)); }} placeholder="No shipping method" /><Input label="Shipping Charge" type="number" min="0" step="0.01" value={shippingCharge} onChange={(event) => setShippingCharge(event.target.value)} /><Input label="Tax" type="number" min="0" step="0.01" value={tax} onChange={(event) => setTax(event.target.value)} /><Input label="Coupon Code" value={couponCode} onChange={(event) => setCouponCode(event.target.value)} /><Input label="Coupon Discount" type="number" min="0" step="0.01" value={couponDiscount} onChange={(event) => setCouponDiscount(event.target.value)} /><Input label="Additional Discount" type="number" min="0" step="0.01" value={additionalDiscount} onChange={(event) => setAdditionalDiscount(event.target.value)} /><FieldSelect label="Payment Method" value={paymentMethod} values={options.payment_methods.map((item) => [item.gateway, item.name])} onChange={setPaymentMethod} /><FieldSelect label="Payment Status" value={paymentStatus} values={options.statuses.payment.map((item) => [item, label(item)])} onChange={setPaymentStatus} /><FieldSelect label="Order Status" value={orderStatus} values={options.statuses.order.map((item) => [item, label(item)])} onChange={setOrderStatus} /></div><div className="mt-3 grid gap-3 md:grid-cols-3"><TextArea label="Customer Notes" value={customerNotes} onChange={setCustomerNotes} /><TextArea label="Delivery Notes" value={deliveryNotes} onChange={setDeliveryNotes} /><TextArea label="Internal Admin Notes" value={adminNotes} onChange={setAdminNotes} /></div></Panel><Panel title="Order Summary"><Summary label="Subtotal" value={subtotal} /><Summary label="Item Discount" value={-itemDiscount} /><Summary label="Coupon Discount" value={-Number(couponDiscount || 0)} /><Summary label="Additional Discount" value={-Number(additionalDiscount || 0)} /><Summary label="Shipping" value={Number(shippingCharge || 0)} /><Summary label="Tax" value={Number(tax || 0)} /><Summary label="Grand Total" value={total} strong /></Panel></div>
  </form>;
}

function ProductPicker({ value, onSelect }: { value: CreateOrderProduct | null; onSelect: (product: CreateOrderProduct) => void }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      setLoading(true);
      try { setResults((await orderManagementService.searchProducts(search)).data.products); } catch { setResults([]); } finally { setLoading(false); }
    }, 300);
    return () => window.clearTimeout(timer);
  }, [open, search]);
  return <label className="relative block space-y-2 text-sm font-semibold"><span>Product</span><div className="flex h-11 items-center gap-2 rounded-lg border border-border bg-background px-3"><Search className="h-4 w-4 text-muted-foreground" /><input value={search} onFocus={() => setOpen(true)} onChange={(event) => { setSearch(event.target.value); setOpen(true); }} placeholder="Search product by name or SKU" className="min-w-0 flex-1 bg-transparent text-sm outline-none" /></div>{open ? <div className="absolute left-0 right-0 top-full z-40 mt-1 max-h-64 overflow-y-auto rounded-lg border border-border bg-background p-1 shadow-xl">{loading ? <p className="p-3 text-xs text-muted-foreground">Searching products...</p> : results.length ? results.map((product) => <button key={product.id} type="button" onMouseDown={(event) => event.preventDefault()} onClick={() => { onSelect(product); setSearch(product.name); setOpen(false); }} className="flex w-full items-center justify-between gap-3 rounded-md px-3 py-2 text-left hover:bg-muted"><span><span className="block font-semibold">{product.name}</span><span className="block text-xs text-muted-foreground">{product.sku ?? "No SKU"} · Stock {product.stock ?? 0}</span></span><span className="shrink-0 text-xs font-bold">{formatPrice(product.price)}</span></button>) : <p className="p-3 text-xs text-muted-foreground">No products found.</p>}</div> : null}</label>;
}

function OrderItemRow({ item, onChange, onRemove }: { item: DraftItem; onChange: (key: string, patch: Partial<DraftItem>) => void; onRemove: (key: string) => void }) {
  const product = item.product;
  return <div className="grid gap-3 rounded-lg border border-border p-3 md:grid-cols-[1.5fr_1fr_90px_110px_110px_auto] md:items-end"><ProductPicker value={product} onSelect={(selected) => onChange(item.key, { product: selected, product_variant_id: "", unit_price: String(selected.price) })} /><FieldSelect label="Variant" value={item.product_variant_id} disabled={!product || product.variants.length === 0} values={(product?.variants ?? []).map((variant) => [String(variant.id), `${variant.label} · ${variant.sku ?? "No SKU"} · Stock ${variant.stock ?? 0}`])} onChange={(value) => { const variant = product?.variants.find((entry) => String(entry.id) === value); onChange(item.key, { product_variant_id: value, unit_price: String(variant?.price ?? product?.price ?? 0) }); }} placeholder={product?.variants.length ? "Select variant" : "Default"} /><Input label="Qty" type="number" min="1" value={item.quantity} onChange={(event) => onChange(item.key, { quantity: event.target.value })} /><Input label="Unit Price" type="number" min="0" step="0.01" value={item.unit_price} onChange={(event) => onChange(item.key, { unit_price: event.target.value })} /><Input label="Discount" type="number" min="0" step="0.01" value={item.discount} onChange={(event) => onChange(item.key, { discount: event.target.value })} /><Button type="button" size="icon" variant="ghost" icon={<Trash2 className="h-4 w-4" />} aria-label="Remove product" onClick={() => onRemove(item.key)} /></div>;
}
