...
export function ProductCard({ product, className, layout = 'grid' }: ProductCardProps) {
  ...
  return (
    <Link
      href={`/products/${product.slug}`}
      className={cn(
        'group relative flex h-full flex-col overflow-hidden rounded-2xl border border-border bg-card transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg',
        className
      )}
    >
      {/* Image */}
      <div className="relative aspect-square overflow-hidden bg-muted">
        ...
        {product.badge && (
          <div
            className={cn(
              'absolute top-3 left-3 px-2 py-1 rounded-lg text-xs font-bold uppercase tracking-wide z-10',
        ...
        {product.isFlashSale && (
          <div className="absolute top-3 right-3 bg-rose-500 text-white p-1 rounded-lg z-10">
        ...
        <div className="absolute bottom-3 left-0 right-0 flex translate-y-0 gap-2 px-3 opacity-100 transition-all duration-300 sm:translate-y-10 sm:opacity-0 sm:group-hover:translate-y-0 sm:group-hover:opacity-100">
          <button
            onClick={handleAddToCart}
            className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-background/95 py-2 text-xs font-semibold text-foreground shadow-lg backdrop-blur-sm transition-colors hover:bg-primary hover:text-primary-foreground"
          >
